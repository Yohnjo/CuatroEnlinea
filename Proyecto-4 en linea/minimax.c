...
void CopiaTablero(char tablero1[N][N], char tablero2[N][N]){ //Funcion para copiar una matriz 
	int i; int j;
	for(i=0;i<N;i++){
		for(j=0;j<N;j++){
...
void BorraArbol(Nodo *raiz){//liberar memoria del arbol cuando ya no se usa
	if(raiz->num_hijos == 0){
        free(raiz);
	}
	else{
...
int ElegirTirada(char partida[N][N], int dificultad){//funcion para elegir la mejor tirada posible segun el minimax, este devuelve el movimiento a ingresar en el tablero
